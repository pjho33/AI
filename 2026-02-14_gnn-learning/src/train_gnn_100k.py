...
def load_and_prepare_data_100k(json_file: str):
    """100k 데이터 로드 및 준비"""
    
    print("="*70)
    print("대규모 데이터 준비 (100,000개)")
    print("="*70)
    ...
    print(f"✓ {len(graphs):,}개 그래프 생성 ({failed}개 실패, {failed/len(reactions)*100:.1f}%)")
    
    return graphs


def train_100k_model(graphs, epochs=100, batch_size=128):
    """100k 모델 학습"""
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), 'data/best_gnn_100k.pt')
        else:
            patience_counter += 1
    ...
    print(f"모델 저장: data/best_gnn_100k.pt")
...
def main():
    ...
    # 데이터 준비
    graphs = load_and_prepare_data_100k('data/uspto_official_100k.json')
    
    # 학습
    model, test_acc = train_100k_model(graphs, epochs=100, batch_size=128)
